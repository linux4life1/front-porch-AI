...
String? waifuDeniedCommand(String command) {
...
class WaifuPermissions {
  WaifuPermissions({this.mode = WaifuMode.build});
  ...
  String? hardBlock({
    ...
    if (path != null && waifuIsEnvPath(path)) {
      return 'denied: .env files are not readable or writable by Waifu Coder';
    }
    if (canon == kWaifuToolBash) {
      final cmd = args['command']?.toString() ?? args['cmd']?.toString() ?? '';
      final denied = waifuDeniedCommand(cmd);
      if (denied != null) return denied;
    }
    if (mode == WaifuMode.plan && waifuToolMutates(canon)) {
  ...
  bool needsAsk({required String name, required Map<String, dynamic> args}) {
    if (!waifuToolMutates(name)) return false;
