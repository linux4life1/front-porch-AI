...
const kDeskSkillSources = <DeskSkillSource>[
  DeskSkillSource(
    id: 'anthropic',
    label: 'Anthropic',
...
class DeskMarketSkill {
  const DeskMarketSkill({
    required this.name,
...
typedef DeskHttpGet = Future<http.Response> Function(Uri uri);

/// Official Claude skills marketplace: github.com/anthropics/skills.
/// HTTPS only — no git clone, no plugin CLI.
class DeskSkillMarket {
  DeskSkillMarket({DeskHttpGet? get}) : _get = get ?? _defaultGet;

  final DeskHttpGet _get;
  List<DeskMarketSkill> catalog = const [];

  static Future<http.Response> _defaultGet(Uri uri) =>
      http.get(uri, headers: _ua);

  Future<List<DeskMarketSkill>> refresh() async {
    final seen = <String>{};
    final out = <DeskMarketSkill>[];
    final errors = <String>[];
  ...
  Future<List<DeskMarketSkill>> _list(DeskSkillSource src) async {
  ...
  Future<String> install({
    ...
    if (!deskSkillNameOk(name)) return 'skill_install: bad name';
    DeskSkillSource? preferred;
...
class DeskSkillHub {
  DeskSkillHub({
    ...
    this.userSkillsDir,
    DeskSkillMarket? market,
  }) : market = market ?? DeskSkillMarket();

  final String projectRoot;
  final String? userSkillsDir;
  final DeskSkillMarket market;
  List<DeskSkillMeta> installed = const [];
  String error = '';
  ...
  String get catalogPrompt =>
      deskSkillCatalogPrompt(installed, oursDir: destDir);

  Set<String> get installedNames => {for (final s in installed) s.name};

  bool _ours(DeskSkillMeta s) => deskSkillIsOurs(s.filePath);

  List<DeskSkillMeta> get porchInstalled => [
    for (final s in installed)
      if (_ours(s)) s,
  ];

  List<DeskSkillMeta> get diskInstalled => [
    for (final s in installed)
      if (!_ours(s)) s,
  ];

  Future<void> refreshLocal() async {
    await deskEnsureSkillsDir(destDir);
    ...
    installed = await deskListLocalSkills(projectRoot, storeDir: userSkillsDir);
  }
  ...
  Future<String> load(String name) =>
      deskLoadSkill(projectRoot, name, storeDir: userSkillsDir);
