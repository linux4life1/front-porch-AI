...
void main() {
  ...
  setUp(() async {
    root = await Directory.systemTemp.createTemp('waifu_e_holes_');
  });
  ...
  test('@.env does not attach secrets', () async {
    ...
    final attached = await waifuExpandMentions('read @.env', root.path);
    expect(attached, isNot(contains('SECRET')));
  });

  test('huge @file is clipped', () async {
    final huge = 'x' * (kWaifuReadClipChars + 5000);
    await File(p.join(root.path, 'big.txt')).writeAsString(huge);
    final attached = await waifuExpandMentions('see @big.txt', root.path);
    expect(attached.length, lessThan(kWaifuReadClipChars + 200));
    expect(attached, contains('clipped'));
  });

  test('skill name with .. does not escape the jail', () async {
    final leaked = await waifuLoadSkill(root.path, '../../etc/passwd');
