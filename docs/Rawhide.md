...
- 🖥️ **Desk (fetch / search / MCP)** — Sit down with a coworker and she loops tools in that folder. She can fetch a URL (redirects refused, marked untrusted), search if Front Porch search is wired, and use MCP tools only if you opt in (the folder jail does not cover remote MCP). Todos, `@file`, `/init`, undo, Plan/Build/Yolo still apply. Not Claude Code; not for code you cannot afford to lose. Desktop only.
