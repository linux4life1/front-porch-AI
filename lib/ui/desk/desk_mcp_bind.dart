...
({List<Map<String, dynamic>> tools, DeskMcpCallFn? call}) deskMcpBind(
  ...
  try {
    ...
    return (
      tools: deskKeepMcpTools(catalog.toOpenAiTools()),
      call: (name, args) async {
        ...
        if (serverId == null) {
          return DeskToolResult.error('mcp: unknown tool $name');
        }
        ...
        return DeskToolResult(
          ...
          output: deskSanitizeMcpOutput(result.text),
        );
      },
    );
...
/// Wikipedia/Tavily via the same client character chat uses. Each desk
/// tool call resets the per-send HTTP cap so a turn can look up more
/// than once (Flutter version AND a package).
DeskWebSearchFn? deskWebSearchBind(BuildContext context) {
  try {
    final chat = Provider.of<ChatService>(context, listen: false);
    return (q) async {
