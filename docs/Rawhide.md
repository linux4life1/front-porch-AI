...
- 🧩 **Worker backend for side jobs** — feelings, wiki/web lookup, journal, and growth can use a second host (Nano, OpenRouter, Kobold, LM Studio, oMLX, or any OpenAI-compatible URL) while chat speech stays on your main model. Two cloud models are fine, including two models on the same host. Two local engines take turns on the GPU: side jobs keep the worker loaded, and chat speech’s model comes back only when she talks (not after every check). A local host with no unload path still stays on chat speech. Leave the worker off and everything stays on one backend. Same on the phone.
