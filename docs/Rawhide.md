...
- 🔧 **Waifu Coder tool rows appear while they work** — a spinner shows on the tool log as soon as they start a call; it flips to the usual pass/fail dot when that call finishes. Tasks they mark completed are struck through in the sidebar. Claiming a todo is done without a real `todowrite` leaves a clear note instead of a silent lie.
