...
void main() {
  ...
  test('onboarding text in an MCP result is dropped', () {
    ...
    final out = waifuSanitizeMcpOutput(raw);
  ...
  test('advertised tools keep Waifu Coder glob and drop list_directory', () {
    ...
    expect(names, contains(kWaifuToolGlob));
    expect(names, contains(kWaifuToolTask));
    expect(names, contains(kWaifuToolWorkflow));
  ...
  test('list_directory is glob, never an MCP call', () async {
    final root = await Directory.systemTemp.createTemp('waifu_dc_');
    addTearDown(() async {
    ...
    expect(canonicalWaifuToolName('list_directory'), kWaifuToolGlob);
    ...
    expect(session.toolChips.any((c) => c.name == kWaifuToolGlob), isTrue);
  });
}
