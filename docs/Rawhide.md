...
- 🧩 **Realism evals sit under Chat speech** — Settings → Backend is one chat stack (host, key, check, model) first. Below that, Realism evals can follow the chat host or use a different one. Same host does not ask for a second key. Two cloud models are fine, including two models on the same host. Two local engines take turns on the GPU: Realism evals keep the worker loaded, and chat speech’s model comes back only when she talks (not after every check). A local host with no unload path still stays on chat speech. Leave the worker off and everything stays on one backend. Same on the phone.

- 🔄 **Two Kobold files on one engine** — Chat speech and Realism evals can each pick a GGUF. Same file stays loaded. Different files unload and reload on the one KoboldCPP the app already starts — no second port. Evals stay on their file until she talks. Same on the phone.
