...
class WaifuDirEntry {
  const WaifuDirEntry({required this.name, required this.path});
...
class WaifuFolderListing {
  ...
  List<WaifuDirEntry> visible({bool includeHidden = false}) => [
...
Future<WaifuFolderListing> listWaifuDirectory(String path) async {
  final parent = p.dirname(path);
  final parentPath = parent == path ? null : parent;
  final dir = Directory(path);
  if (!await dir.exists()) {
    return WaifuFolderListing(
      path: path,
  ...
  final directories = <WaifuDirEntry>[];
  await for (final entity in dir.list(followLinks: false)) {
    if (entity is! Directory) continue;
    directories.add(
      WaifuDirEntry(name: p.basename(entity.path), path: entity.path),
    );
  }
  directories.sort(
  ...
  final hints = <String>[];
  for (final marker in kWaifuProjectMarkers) {
