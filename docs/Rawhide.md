...
- 🧩 **Side jobs sit under Chat speech** — Settings → Backend is one chat stack (host, key, check, model) first. Below that, Side jobs can follow the chat host or use a different one. Same host does not ask for a second key. Two cloud models are fine, including two models on the same host. Two local engines take turns on the GPU: side jobs keep the worker loaded, and chat speech’s model comes back only when she talks (not after every check). A local host with no unload path still stays on chat speech. Leave the worker off and everything stays on one backend. Same on the phone.
