...
void main() {
  ...
  setUp(() async {
    root = await Directory.systemTemp.createTemp('desk_g_');
  });
  ...
  test('over-budget transcript is compacted without inventing files', () {
    final msgs = <DeskMessage>[
      for (var i = 0; i < 40; i++)
        DeskMessage(isUser: i.isEven, text: 'line $i ' * 20),
    ];
    final compact = deskCompactTranscript(msgs, budgetChars: 200, keep: 4);
  ...
  test('resume restores folder, coworker, title, and transcript', () async {
    ...
    session.transcript.add(const DeskMessage(isUser: true, text: 'add hello'));
    session.transcript.add(
      const DeskMessage(isUser: false, text: 'Hmph. Done.'),
    );
    ...
    expect(loaded.mode, DeskMode.yolo);
  });
  ...
  test('send compact+save uses the store and sets a title', () async {
    ...
    final harness = DeskHarness(session: session, llm: llm, store: store);
  ...
  test('saveLast does not dump firstMes or other card secrets', () async {
    ...
    final store = DeskStore(storeDir.path);
    await store.saveLast(session);
    final raw = await File(p.join(storeDir.path, kDeskLastFile)).readAsString();
    expect(raw, isNot(contains('SECRET_GREETING')));
    expect(raw, isNot(contains('SECRET_SCENARIO')));
    expect(raw, contains('tsundere'));
  });

  test('corrupt last_desk.json loads as null', () async {
    final storeDir = Directory(p.join(root.path, 'store'))..createSync();
    File(p.join(storeDir.path, kDeskLastFile)).writeAsStringSync('{not json');
    expect(await DeskStore(storeDir.path).loadLast(), isNull);
  });

  test('deskStoreDirectory is dataRoot/desk', () {
    expect(
      p.equals(deskStoreDirectory('/data'), p.join('/data', 'desk')),
      isTrue,
    );
  });
}
