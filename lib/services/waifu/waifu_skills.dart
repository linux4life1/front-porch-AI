...
bool waifuSkillNameOk(String name) {
  final t = name.trim();
...
(String name, String description) waifuParseSkillFrontmatter(String body) {
  var name = '';
  var description = '';
...
String? waifuUserSkillsDir({String? home}) {
  if (Platform.environment.containsKey('FLUTTER_TEST')) return null;
  final h = home ?? waifuHomeDir();
  if (h == null || h.isEmpty) return null;
  return p.join(h, kWaifuDotDir, 'skills');
}

bool waifuSkillIsOurs(String filePath) {
...
List<String> waifuOursSkillRoots(String projectRoot, {String? storeDir}) {
  final user = storeDir ?? waifuUserSkillsDir();
  return [
...
List<String> waifuOtherSkillRoots(String projectRoot, {String? home}) {
  final h = home ?? waifuHomeDir();
  return [
...
List<String> waifuSkillSearchRoots(
  String projectRoot, {
  String? storeDir,
  bool others = true,
}) {
  ...
  final out = <String>[];
  for (final root in [
    ...waifuOursSkillRoots(projectRoot, storeDir: storeDir),
    if (others) ...waifuOtherSkillRoots(projectRoot),
  ]) {
...
Future<void> waifuEnsureSkillsDir(String dir) async {
...
Future<List<WaifuSkillMeta>> waifuListLocalSkills(
  String projectRoot, {
  String? storeDir,
}) async {
  final seen = <String>{};
  final out = <WaifuSkillMeta>[];
  for (final root in waifuOursSkillRoots(projectRoot, storeDir: storeDir)) {
...
Future<void> _walkSkillDir(
  ...
  required List<WaifuSkillMeta> out,
  required int depth,
}) async {
  if (depth > kWaifuSkillScanMaxDepth) return;
  ...
  await for (final entity in dir.list(followLinks: false)) {
    ...
    if (await file.exists()) {
      if (!waifuSkillNameOk(name) || seen.contains(name)) continue;
      String body;
      try {
      ...
      final (frontName, description) = waifuParseSkillFrontmatter(body);
      seen.add(name);
      out.add(
        WaifuSkillMeta(
...
String waifuSkillCatalogPrompt(List<WaifuSkillMeta> skills, {String? oursDir}) {
...
Future<String> waifuLoadSkill(
  String root,
  String name, {
  String? storeDir,
}) async {
  final trimmed = name.trim();
  if (!waifuSkillNameOk(trimmed)) {
    return 'skill not found: $name';
  }
  for (final base in waifuSkillSearchRoots(root, storeDir: storeDir)) {
    final file = await _locateSkillFile(base, trimmed);
    if (file == null) continue;
    try {
      var body = await file.readAsString();
      if (body.length > kWaifuReadClipChars) {
        body = '${body.substring(0, kWaifuReadClipChars)}\n…(clipped)';
      }
      return body;
    } catch (_) {}
  }
  ...
  for (final rel in legacy) {
    final hit = await WaifuJail.resolveLive(root, rel);
    if (!hit.ok) continue;
...
Future<File?> _locateNamed(Directory dir, String name, int depth) async {
  if (depth > kWaifuSkillScanMaxDepth) return null;
  await for (final entity in dir.list(followLinks: false)) {
    if (entity is! Directory) continue;
