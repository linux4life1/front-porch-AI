...
- 🖥️ **Waifu Coder** — sit down with a character who codes: Plan/Build, in-folder edits without nagging, tasks saved under `.waifu`, they review and test before they talk, and that spoken line ends the turn. The first move on a code change is a tool, not a ten-step think. “Done” only counts after a real write and a passing check. The context bar is the real request (system, tools, and the model's own token count). Old turns fold into a recap when the window fills, like other coding agents. Desktop only.
