...
void main() {
  ...
  setUp(() async {
    root = await Directory.systemTemp.createTemp('desk_f_');
  });
  ...
  test('MCP tools are not advertised unless opted in', () async {
    ...
    final llm = ScriptedDeskLlm([const LlmToolResponse(calls: [], text: 'hi')]);
    final session = DeskSession(
      folderRoot: root.path,
      coworker: _mira(),
      mode: DeskMode.yolo,
    );
    final off = DeskHarness(
      session: session,
      llm: llm,
      mcpTools: [ping],
      mcpOptIn: false,
    ...
    final on = DeskHarness(
      ...
      mcpCall: (name, args) async =>
          const DeskToolResult(ok: true, output: 'pong'),
    );
    await on.send('hello');
    final namesOn = [
      for (final t in llm.calls.single.tools)
