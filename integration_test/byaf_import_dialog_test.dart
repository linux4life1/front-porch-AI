...
// HomePageDialogs._importByaf really deletes the temp extracts and calls
// CharacterRepository.addLook — nothing about that behavior is reproduced
// or stubbed here.
//
...
//      is added, and the temp extract is still deleted (the dialog's own
//      code path in home_page_dialogs.dart deletes it unconditionally,
//      independent of the checkbox).
//
// The fixture images are a real minimal decodable PNG (not the single-byte
// stand-ins byaf_service_test.dart uses) — the confirmed path round-trips
// the portrait through V2CardService's image decode/re-encode, which a
// non-image byte string would only survive by accident (falling back to a
// synthesized placeholder), masking exactly the behavior under test.
//
// Run it with:
//   flutter test integration_test/byaf_import_dialog_test.dart -d windows
//
...
/// img.decodeImage, unlike byaf_service_test.dart's `[i]` stand-ins.
const _kTinyPngBase64 =
    'iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAIAAABLbSncAAAAEUlEQVR42mP4/'
    'vU9VsQwtCQAafG2wXWW5mYAAAAASUVORK5CYII=';

const _kGalleryToggleText = 'Import gallery images (1 extra looks)';
...
/// Opens the real BYAF import flow (HomePageDialogs._importByaf) — an empty
/// library shows a direct "Import BYAF" button on the empty-state screen,
/// while a non-empty one only offers it through the grid toolbar's import
/// menu (tooltip 'Import or discover characters' → 'Import Backyard AI
/// (.byaf)'). Both call the identical private method; this just finds
/// whichever entry point the current library state renders.
...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      // The sandboxed path_provider temp dir ByafService.parseByaf extracts
      // into — SandboxPathProvider._dir('tmp') creates exactly this path.
      ...
      // ── Scenario 1: Cancel — even with the checkbox left checked ────────
      ...
      // ── Scenario 2: Confirm, gallery checkbox CHECKED (default) ─────────
      ...
      // ── Scenario 3: Confirm, gallery checkbox UNCHECKED ──────────────────
      currentFixturePath = _writeByaf(
        fixtureDir,
        'unchecked.byaf',
        'Unchecked Case',
