...
void main() {
  testWidgets('typing / lists slash commands with blurbs', (tester) async {
    ...
    await tester.pump();
    expect(find.byKey(const Key('desk-slash-menu')), findsOneWidget);
    expect(find.byKey(const Key('desk-slash-help')), findsOneWidget);
    expect(find.textContaining('List every slash command'), findsOneWidget);
    await tester.tap(find.byKey(const Key('desk-slash-help')));
    await tester.pump();
  ...
  testWidgets('sidebar shows used vs max context', (tester) async {
    final session =
        DeskSession(
          ...
          ..contextBudget = 8192;
    await tester.pumpWidget(MaterialApp(home: DeskPage(session: session)));
    expect(find.byKey(const Key('desk-context-bar')), findsOneWidget);
