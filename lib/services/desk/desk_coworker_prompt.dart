...
const kDeskTalkSampleMaxTokens = 400;
const kDeskTalkSampleMaxCount = 2;
const kDeskPersonaFallbackMaxChars = 400;

String deskTodayStamp([DateTime? now]) {
...
String deskFillCharMacros(String raw, String name) {
...
/// One or two mes_example slices, capped at [kDeskTalkSampleMaxTokens].
String? deskTalkFeel(String mesExample, String name) {
  final filled = deskFillCharMacros(mesExample, name).trim();
  if (filled.isEmpty) return null;
  final chunks = filled
      .split(RegExp(r'(?:<START>|\n{2,})', caseSensitive: false))
      .map((s) => s.trim())
      .where((s) => s.isNotEmpty)
      .take(kDeskTalkSampleMaxCount)
      .toList();
  if (chunks.isEmpty) return null;
  final buf = StringBuffer();
  var tokens = 0;
  for (final chunk in chunks) {
    if (tokens >= kDeskTalkSampleMaxTokens) break;
    var piece = chunk;
    final roomTokens = kDeskTalkSampleMaxTokens - tokens;
    if (deskEstimateTokens(piece) > roomTokens) {
    ...
    buf.writeln(piece);
    tokens = deskEstimateTokens(buf.toString());
  }
...
String buildDeskCoworkerPrompt(CharacterCard card, {DateTime? now}) {
  ...
  if (personality.isNotEmpty) {
    ...
    final clip = description.length <= kDeskPersonaFallbackMaxChars
        ? description
        : '${description.substring(0, kDeskPersonaFallbackMaxChars).trimRight()}…';
    buf.writeln('Persona: $clip');
  }
  final talk = deskTalkFeel(card.mesExample, card.name);
  if (talk != null) {
    buf
      ..writeln()
      ..writeln(
  ...
  buf
    ..writeln()
    ..writeln('Today: ${deskTodayStamp(now)}')
...
String deskLoopUserPrompt({
  required String folderName,
  required String coworkerName,
  required List<DeskMessage> transcript,
  ...
  final buf = StringBuffer()
    ...
    ..writeln(kDeskLookupCue)
    ..writeln(kDeskNestCue)
    ..writeln();
