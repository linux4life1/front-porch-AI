...
bool backfillSlotClocks(
  ...
  bool repairInvertedPair(
    ...
    // A skip whose snap is older than after is not inverted — after
    // is the skip landing. Collapsing it to before undoes the skip.
    if ((slot?['time_skip_to'] as String?)?.isNotEmpty == true) {
      final snap = slotSnapClock(slot);
      final after = slotClockAfter(slot);
      if (snap != null && after != null && snap.isBefore(after)) {
        return false;
      }
    }
