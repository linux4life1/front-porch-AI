...
void main() {
  ...
  setUp(() async {
    root = await Directory.systemTemp.createTemp('desk_undo_ui_');
  });
  ...
  testWidgets('Undo and Redo keys exist; Continue and Regen do not', (
    ...
    expect(find.byKey(const Key('desk-undo')), findsOneWidget);
    expect(find.byKey(const Key('desk-redo')), findsOneWidget);
    ...
    await tester.pump();
    expect(
      tester.widget<IconButton>(find.byKey(const Key('desk-undo'))).onPressed,
      isNotNull,
    );
    ...
    await tester.pump();
    expect(
      tester.widget<IconButton>(find.byKey(const Key('desk-redo'))).onPressed,
      isNotNull,
    );
  });
}
