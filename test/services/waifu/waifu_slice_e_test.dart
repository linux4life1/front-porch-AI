...
void main() {
  ...
  setUp(() async {
    root = await Directory.systemTemp.createTemp('waifu_e_');
  });
  ...
  test('todowrite stores items; todoread returns them', () async {
    ...
    final harness = WaifuHarness(
      session: _session(root.path, WaifuMode.yolo),
      llm: llm,
    );
  ...
  test('skill injects SKILL.md from .waifu/skills', () async {
    final skillDir = Directory(p.join(root.path, '.waifu', 'skills', 'review'));
