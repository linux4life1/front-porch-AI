...
- 🖥️ **Desk (resume + compact)** — Sit down with a coworker and she loops tools in that folder. Long sessions fold old turns into a recap (no invented files). The last sit-down comes back as Resume on the Desk home tab. Fetch/search/MCP, todos, `@file`, `/init`, undo, and Plan/Build/Yolo still apply. Not Claude Code; not for code you cannot afford to lose. Desktop only.
