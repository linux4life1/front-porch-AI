...
/// Their FS/shell tools bypass DeskJail; their descriptions dump
/// another product's onboarding into the spoken line.
const kDeskMcpBlockedBareNames = {
  'list_directory',
  'directory_tree',
  'read_file',
  'write_file',
...
String deskBareMcpName(String name) {
  var n = name.trim();
...
bool deskMcpNameBlocked(String name) =>
    kDeskMcpBlockedBareNames.contains(deskBareMcpName(name));

bool deskMcpToolBlocked(Map<String, dynamic> tool) {
  final fn = tool['function'];
  if (fn is! Map) return true;
  final name = (fn['name'] ?? '').toString();
  final desc = (fn['description'] ?? '').toString();
  if (deskMcpNameBlocked(name)) return true;
...
String deskSanitizeMcpOutput(String raw) {
...
String deskSanitizeMcpDescription(String raw) {
  final cut = RegExp(r'\n?\s*New to\b', caseSensitive: false).firstMatch(raw);
  final t = cut == null ? raw : raw.substring(0, cut.start);
  return t.trim();
}

Map<String, dynamic> deskSanitizeMcpTool(Map<String, dynamic> tool) {
  final fn = tool['function'];
  if (fn is! Map) return tool;
  final desc = deskSanitizeMcpDescription((fn['description'] ?? '').toString());
  return {
    ...tool,
...
List<Map<String, dynamic>> deskKeepMcpTools(List<Map<String, dynamic>> tools) {
  return [
    for (final t in tools)
      if (!deskMcpToolBlocked(t)) deskSanitizeMcpTool(t),
  ];
}
