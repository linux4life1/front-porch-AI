...
import 'package:front_porch_ai/ui/desk/desk_wizard_sit_down_step.dart';
import 'package:front_porch_ai/ui/theme/app_colors.dart';

/// Project → Coworker → Sit down. Create Character chrome, linear only.
class DeskWizardPage extends StatefulWidget {
  const DeskWizardPage({
    super.key,
    this.characters,
  ...
  final Future<DeskFolderListing> Function(String path)? listDirectory;

  @override
  State<DeskWizardPage> createState() => _DeskWizardPageState();
}

class _DeskWizardPageState extends State<DeskWizardPage> {
  static const _labels = ['Project', 'Coworker', 'Sit down'];

  int _currentStep = 0;
  late String _path;
  DeskFolderListing? _listing;
  bool _loading = true;
  bool _folderConfirmed = false;
  CharacterCard? _coworker;
  bool _honesty = false;
  DeskMode _mode = DeskMode.build;
  ...
  Future<void> _load() async {
    ...
    final listFn = widget.listDirectory ?? listDeskDirectory;
  ...
  Widget _body() {
    switch (_currentStep) {
      ...
      case 1:
        return DeskWizardCoworkerStep(
          characters: _cards,
          selected: _coworker,
          onSelected: (c) => setState(() => _coworker = c),
        );
      default:
        return DeskWizardSitDownStep(
          folderPath: _path,
          coworker: _coworker,
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              key: const Key('desk-wizard-next'),
              onPressed: _canNext
