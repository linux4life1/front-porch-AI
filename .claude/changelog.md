...
## 2026-06-06 (Rawhide CI: V2.5 seed-doubling regression + stale one-shot test)
- Branch: `fix/rawhide-realism` (off `upstream/Rawhide`). Fixes 2 red tests in `test/services/chat_service_realism_engine_test.dart`. Classified first (stale test vs real regression), then fixed.
- **Failure 1 — REAL REGRESSION (prod code).** `startNewChat`'s 1:1 V2.5 seeding ran the legacy ±150→±300 migration (`_migrateShortTermScore`/`_migrateLongTermScore`, which *2 any |value|<=150) on the card's `shortTermBond`/`longTermBond`. But card seeds are authored on the current ±300 scale (FrontPorchExtensions docs: "-300 to 300"), so this double-counted: a card authored at bond 55 started every new chat at 110. `trustLevel` in the same block was (correctly) never migrated, confirming the inconsistency. The migration leaked into the card-seed path via merge 8fc8ea7; it is only valid for upgrading previously-persisted *session* scores (still correctly used in _loadLastSession:3306/3308 and loadSession). Fix: seed via plain `.clamp()` only — no migration. The migration helpers stay (legacy session loads still use them). Test expectation (55) was correct.
- **Failure 2 — STALE TEST (test only).** The one-shot eval path fires and applies deltas correctly (verified: `[Realism:OneShot] Evaluating (fused call)...` → `Trust shifted by 12`). Test failed only because it asserted the one-shot prompt contained `'autonomous story engine'`/`'one shot'`/`'bond_delta'` — none exist in `_evaluateOneShotCall`'s prompt (it uses `relationship_delta`/`trust_delta`); `'autonomous story engine'` is the *narrative multi-call* prompt. Updated the assertion to require `relationship_delta` && `trust_delta` (markers actually in the one-shot prompt). No prod change.
- **Out of scope / flagged, NOT fixed:** the file has pre-existing async-teardown flakiness — `ChatService._loadActiveObjectives` fires `notifyListeners()` after a prior test disposed its service (`ChangeNotifier used after disposed`), causing intermittent failures whose victim varies run-to-run (seeding/one-shot/group). Reproduces on clean `upstream/Rawhide` (verified via detached worktree, multiple runs). It is timing-dependent and has not been observed to fail CI, so it does not block these two fixes from greening CI. Belongs in a separate test-infra branch (await/cancel pending async before dispose), not this parity-sensitive change.
- Files: `lib/services/chat_service.dart` (seed migration removed on 1:1 path), `test/services/chat_service_realism_engine_test.dart` (one-shot marker), `docs/Rawhide.md`, `.claude/changelog.md`
- Hygiene: New private methods: 0. Methods deleted: 0 (both `_migrate*` helpers still used by legacy session-load paths — confirmed via grep, not dead). `flutter analyze` on both changed files: clean (only 8 pre-existing `info` doc-comment HTML notices at unrelated lines; none on changed lines, none introduced). Both target tests pass in isolation; group/seeding flakiness characterized as pre-existing (baseline worktree comparison). Realism parity: change is gated on `_activeGroup == null` (1:1 only); group seeding path untouched.
