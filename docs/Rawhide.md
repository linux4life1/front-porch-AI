...
- 🖥️ **Waifu Coder** — sit down with a character who codes: Plan/Build, in-folder edits without nagging, tasks saved under `.waifu`, they review and test before they talk, and that spoken line ends the turn. The first move on a code change is a tool, not a ten-step think. “Done” only counts after a real write and a passing check. She does not re-read a file that is already in the prompt. The context bar is the real request (system, tools, and the model's own token count). Old turns fold into a recap when the window fills, like other coding agents. Plan/Build/Yolo is one control. MCP tools update when you connect a server. Leave Stop. A porch you already sat down on does not re-quiz honesty. Plan titles can have colons. Accepting a plan does not wipe tasks you already had. Desktop only.
