...
- 🖥️ **Desk (todos / ask / @files)** — Sit down with a coworker and she loops tools in that folder. She can keep a todo list, pause to ask you a choice, attach `@file`, `/init` an AGENTS.md, and load a SKILL.md. Commands stay in that folder. Undo is only her writes. Plan cannot write; Build asks. Not Claude Code; not for code you cannot afford to lose. Desktop only.
