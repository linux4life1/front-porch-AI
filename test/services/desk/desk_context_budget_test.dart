...
void main() {
  test('deskMeasurePrompt adds system tokens to the user prompt', () {
    ...
    expect(snap.used, deskEstimateTokens(system) + deskEstimateTokens(prompt));
    expect(snap.used, greaterThan(deskEstimateTokens(prompt)));
  });

  test('harness tokensUsed includes the coworker system prompt', () async {
    final root = await Directory.systemTemp.createTemp('desk_budget_');
    addTearDown(() async {
    ...
    final session = DeskSession(folderRoot: root.path, coworker: coworker);
    await DeskHarness(session: session, llm: llm).send('hi');
    expect(llm.calls, hasLength(1));
    final request = deskMeasurePrompt(
    ...
    expect(
      session.tokensUsed,
      greaterThan(deskEstimateTokens(llm.calls.first.prompt)),
    );
    expect(llm.calls.first.systemPrompt, contains(kDeskPreamble));
  });
}
