...
describe('StoopVerifiedBadge', () => {
  ...
  it('draws the hub path in gold and blue, and nothing otherwise', () => {
    ...
    expect(gold?.getAttribute('d')).toBe(HUB_PATH);
    expect(container.querySelector('[aria-label="Gold verified"]')).not.toBeNull();

    render('blue');
    expect(container.querySelector('.stoop-check-blue svg path')?.getAttribute('d')).toBe(
      HUB_PATH,
    );
