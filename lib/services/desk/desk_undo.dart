...
/// Undo/redo of Desk write/edit records only — not git reset of user work.
class DeskUndo {
  final _undo = <DeskWriteRecord>[];
  final _redo = <DeskWriteRecord>[];

  bool get canUndo => _undo.isNotEmpty;
  bool get canRedo => _redo.isNotEmpty;

  void push(DeskWriteRecord rec) {
    _undo.add(rec);
    _redo.clear();
  }

  Future<DeskWriteRecord?> undo(String root) async {
  ...
  Future<DeskWriteRecord?> redo(String root) async {
  ...
  Future<void> _apply(
    ...
    final hit = await DeskJail.resolveLive(root, relative);
    if (!hit.ok) return;
